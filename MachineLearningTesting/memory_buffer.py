...
class MemoryBuffer(object):
	...
	def __init__(self, buffer_size, with_per = False):
		...
		self.totalMemCount = 0

	def saveMemory(self, state, action, reward, done, new_state, error=None):
		""" Save an experience to memory, optionally with its TD-Error
		"""
		if done != 2:
			self.totalMemCount += 1
			experience = (state, action, reward, 1-done, new_state)
			if(self.with_per):
