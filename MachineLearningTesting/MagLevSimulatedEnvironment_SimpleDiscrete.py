...
class MagLevEnvironment:
	...
	def __init__(self, mass=25, timestep=.001, initCase=0, rewardFunction=0, targetFrequency = 1):
		...
		# 7= init on either upper or lower rails
	...
	def reset(self):
		...
		self.states.append([self.position,self.velocity,self.accel,self.positionError,0,0,0,self.targetState])
	...
	def step(self, action):
		"""
		if action == 0:
			self.current -= 4*self.currentStep
		elif action == 1:
			self.current -= self.currentStep
		elif action == 2:
			self.current = self.current # not needed
		elif action == 3:
			self.current += self.currentStep
		elif action == 4:
			self.current += 4*self.currentStep
		...
		if self.current < -1.5:
			self.current = -1.5
		# PHYSICS UPDATE
		self.accel = self.getAccel(current=self.current,airgap=self.position,mass=self.mass)
		...
		# LOG DATA
		self.states.append([self.position,self.velocity,self.accel,self.positionError,action,reward,self.current,self.targetState])
		# NORMALIZE VALUES
	...
	def getReturnableState(self):
		...
		return ([normalizedPosition,self.velocity,self.accel,normalizedPositionError,self.current])
	...
	def getRewardIdeal(self):
		...
		if abs(self.position-self.targetState) < 10:
			reward = abs(self.position-self.targetState) * 5
		else: # position over 10, get the penalty for that plus the delta penalty at a slower rate
			reward = (abs(self.position-self.targetState)-10) + 50
		
		#reward += abs(self.accel)*100 # accel maxes out at 3.5, but is more typically +/-1, and even less at extremes

		#if self.action == 0 or self.action == 4: # extreme current swing, penalize this
		#	reward += 25

		#if abs(self.velocity) > .5:
		#	reward += (abs(self.velocity)-.5)*50

		"""
		if abs(self.velocity) > 2 and abs(self.velocity) < 5: # penalize velocity, but only over a threshold to encourage that exact speed
			reward += (abs(self.velocity)-2)*50 # gives 100 at V=4, 500 at V=12 and 1000 at V=22, so quickly becomes dominant
		elif abs(self.velocity) >= 5:
			reward += (abs(self.velocity)-5)*10 + 150 # so that's 150 at V=5 and 300 at V=20
		"""
		reward = -reward
		"""
	...
	def plotStates(self):
		...
		for observation in self.states:
			...
			actions.append(observation[4]-5)
			costs.append(observation[5])
			currents.append(observation[6])
			targetStates.append(observation[7])
		plt.plot(positions)
		plt.plot(velocitys)
...
if __name__ == '__main__':
	...
	for counter in range(200):
		...
		obs,r,done,info = env.step(myAction)
	...
	print(env.getAccel(current=0, airgap=180, mass=25))
