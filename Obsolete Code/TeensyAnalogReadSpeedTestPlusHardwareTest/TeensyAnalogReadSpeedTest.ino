...
void setup() {
  ...
  int microsec = 5000000 / leds.numPixels();
  while(1){
...
int intTempLoopCounter = 0;

void loop() {
  if (intTempLoopCounter > 500){digitalWrite(pinIRONOFF, LOW);}
  else {digitalWrite(pinIRONOFF, HIGH);};
  if (intTempLoopCounter > 1000){intTempLoopCounter = 0;}
  //intTempLoopCounter ++;
  
  // sample all analog inputs, Timing ourselves
...
// only supports a single mux (may need to upgrade for the final project). Note there are OOP libraries for analog muxes,
